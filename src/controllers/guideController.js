...
// Multer setup for handling file uploads (temporary storage)
// Configure multer with memory storage for Vercel serverless environment
const upload = multer({
    ...
    limits: { 
        fileSize: 4.5 * 1024 * 1024, // 4.5MB file size limit (Vercel has 4.5MB payload limit)
    ...
    // Apply Vercel body size check before processing the upload
        ...
        try {
            ...
            await guideRef.set({
                ...
                fileId: imageKitResponse.fileId, // Store ImageKit fileId for future reference
                isVerified: false,
                status: 'pending',
            ...
            const msg = {
                ...
                dynamicTemplateData: {
                    ...
                    verificationUrl,
                    rejectionUrl,
                    licenseUrl,// Include the ImageKit URL in the email
                },
            };
...
// New function to delete PDF from ImageKit if needed
